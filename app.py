...
if traefik_routers_response.status_code != 200:
  raise ValueError(f"Failed to make request to Traefik API. Status code: {traefik_routers_response.status_code}")
...
unifi_login_response = unifi_session.post(f"{unifi_url}api/auth/login", json={
...
if get_static_dns_entries_response.status_code != 200:
  raise ValueError(f"Failed to get static DNS entries from Unifi API. Status code: {get_static_dns_entries_response.status_code}")
  ...
  for entry in unifi_static_dns_entries:
    if entry[0] == dns_name:
      ...
      if entry[1] != traefik_ip:
        print(f"DNS name {dns_name} already exists but with different IP address {entry[1]}. Updating it to {traefik_ip}.")
...
for entry in entries_to_update:
  update_static_dns_entry_response = unifi_session.put(f"{unifi_url}proxy/network/v2/api/site/default/static-dns/{entry[1]}", json={
    "enabled": True,
    "key": entry[0],
    "record_type": "A",
    "value": traefik_ip,
  ...
  if update_static_dns_entry_response.status_code == 200:
    ...
    print(f"Failed to update static DNS entry in Unifi API. Status code: {update_static_dns_entry_response.status_code}")

for host in hosts_to_add:
  add_static_dns_entry_response = unifi_session.post(f"{unifi_url}proxy/network/v2/api/site/default/static-dns", json={
    "enabled": True,
    "key": host,
    "record_type": "A",
    "value": traefik_ip
  ...
  if add_static_dns_entry_response.status_code == 200:
    ...
    print(f"Failed to add static DNS entry in Unifi API. Status code: {add_static_dns_entry_response.status_code}")
